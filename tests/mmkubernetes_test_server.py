...
# (argv[4]), the server rejects any request whose "Authorization: Bearer <tok>"
# does not match the file's current contents with HTTP 401. The test rotates
# this file to exercise the token reload-on-401 path in mmkubernetes. When it is
# not set, no auth check is performed and existing tests behave as before.
expected_token_file = None
...
# optional argv[4]: path to the file holding the currently-valid bearer token
if len(sys.argv) > 4:
    expected_token_file = sys.argv[4]
